...
Database::Database(const std::string &dir, size_t memLimit, size_t sstLimit)
    : memtable(), sstables(), flushBuffer(nullptr), memtableLimit(memLimit),
      sstableLimit(sstLimit), directory(dir), wal_(directory + "/wal.log"),
      db_mutex(), mergeInProgress(false) {
...
void Database::recoverFromWAL() {
  wal_.recover([this](const std::string &key, const std::vector<uint8_t> &blob,
                      bool tombstone) {
    std::lock_guard<userver::engine::Mutex> lock(db_mutex);
...
Database::selectInternal(const std::string &key) {
  {
...
void Database::SnapshotCsv(const std::string &csv_path) const {
  ...
  std::unique_ptr<SkipListMap<std::string, DBEntry>> flush_copy;
  {
    ...
    if (flushBuffer) {
      ...
      for (auto it = flushBuffer->begin(); it != flushBuffer->end(); ++it) {
        auto p = *it;
