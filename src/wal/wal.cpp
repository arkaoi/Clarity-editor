...
void WAL::logInsert(const std::string &key,
  ...
  if (out_) {
    ...
    out_->write(reinterpret_cast<const char *>(&valueSize), sizeof(valueSize));
    if (valueSize > 0) {
      out_->write(reinterpret_cast<const char *>(valueBlob.data()), valueSize);
    }
    out_->flush();
  }
}
  ...
  while (!in.eof()) {
    ...
    if (opType == 1) {
      ...
      if (valueSize > 0) {
        ...
        if (!in || in.gcount() != static_cast<std::streamsize>(valueSize)) {
          break;
        }
      }
...
void WAL::clear() {
  ...
  int fd_clear = ::open(filename_.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd_clear >= 0) {
    ::close(fd_clear);
  }
